...
const FourthImage = ({data}) => {
    ...
    let fetchData = async () => {
        const forsetRef = ref(storage, `/event/${currentPath.id}/${data}/activity${currentPath.id}4.png`);
    ...
    let bannerSend = (evt) => {
        const storageRef = ref(storage, `event/${currentPath.id}/${data}/activity${currentPath.id}4.png`);
        const metadata = {
        ...
        return (
            <header className='activityImageChang'>
                {/* 自己頁面的更換 */}
                {
                ...
                <div onClick={bannerShowBtn} className='activityImageChangItem' style={{ backgroundImage: `url('${state.personalBanner}')` }} ></div>
