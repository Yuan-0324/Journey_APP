...
const SecondImage = ({data}) => {
    ...
    let fetchData = async () => {
        const forsetRef = ref(storage, `/event/${currentPath.id}/${data}/activity${currentPath.id}2.png`);
    ...
    let bannerSend = (evt) => {
        const storageRef = ref(storage, `event/${currentPath.id}/${data}/activity${currentPath.id}2.png`);
        const metadata = {
