...
const ThirdImage = ({data}) => {
    ...
    let fetchData = async () => {
        const forsetRef = ref(storage, `/event/${currentPath.id}/${data}/activity${currentPath.id}3.png`);
    ...
    let bannerSend = (evt) => {
        const storageRef = ref(storage, `event/${currentPath.id}/${data}/activity${currentPath.id}3.png`);
        const metadata = {
        ...
        return (
            <header className='activityImageChang'>
                ...
                <div onClick={bannerShowBtn} className='activityImageChangItem' style={{ backgroundImage: `url('${state.personalBanner}')` }} >
                    {/* <div className='header-banner-image'></div> */}
                </div>
