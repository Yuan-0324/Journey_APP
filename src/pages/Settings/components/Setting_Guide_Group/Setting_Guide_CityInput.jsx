...
const Setting_Guide_CityInput = ({ setCitySwitch, guideData, setGuideData }) => {

    const { userInfo } = useContext(context);

    let userEmail = userInfo.email;

    let CityBeforeValue = guideData.City;
    function CityOnChange(value, label, extra) {
        CityBeforeValue = value;
    }

    async function CityOnClick() {
        //將父階的使用者資訊=暫存值
        guideData.City = CityBeforeValue;
        //存進物件 準備丟進put
        let totolCity = {
            city: guideData.City,
            email: userEmail
        ...
        await Axios.put('http://localhost:8000/guide/setCity', totolCity)
            .then((res) => {
                console.log(res);
                setCitySwitch(0);
            })
    ...
    return (
        <>
            ...
            <button onClick={CityOnClick}>修改</button>
            <button onClick={() => {
                setCitySwitch(0);
            }}>取消</button>
        </>
...
export default Setting_Guide_CityInput;